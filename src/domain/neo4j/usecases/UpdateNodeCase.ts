...
import {NodeCreateDTO, NodeD3, NodeUpdateDTO} from "@/domain/neo4j/models/Node";
...
export abstract class UpdateNodeCase implements UseCase<Promise<{ node: NodeD3, relationships: RelationshipD3[] }>> {
  abstract execute(node: NodeD3, updatedNode: NodeUpdateDTO, relationships: RelationshipD3[]): Promise<{ node: NodeD3, relationships: RelationshipD3[] }>
}

export class UpdateNodeCaseImpl implements UpdateNodeCase {
  constructor(private updateNode: Func) {}

  async execute(node: NodeD3, updatedNode: NodeUpdateDTO, relationships: RelationshipD3[]): Promise<{ node: NodeD3, relationships: RelationshipD3[] }> {
    try {
      const update = new NodeD3(await this.updateNode(node.elementId, updatedNode), node.x, node.y)
      return { node: update, relationships: this.parseRelationships(update, relationships) }
    } catch {
