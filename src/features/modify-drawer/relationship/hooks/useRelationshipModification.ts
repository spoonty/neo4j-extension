...
export const useRelationshipModification = (initialRelationship: RelationshipD3) => {
    ...
    const modifyHandler = async () => {
        if (initialRelationship) {
            ...
            await updateRelationship(initialRelationship.elementId, type, convertProperties(properties))
        } else {
    ...
    useEffect(() => {
        updateRelationshipTemplate(type, convertProperties(properties), initialRelationship)
