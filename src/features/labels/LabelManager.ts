...
class LabelManager {
    ...
    addLabel = (label: string) => {
        const labelsColor: KeyValue<string, string> = storageImpl.get(this.STORAGE_KEY) || {};

        if (labelsColor) {
            const labels = Object.keys(labelsColor);

            if (!labels.includes(label)) {
                labelsColor[label] = this.COLORS.find((color) => !Object.values(labelsColor).includes(color)) || this.COLORS[labels.length % this.COLORS.length];
